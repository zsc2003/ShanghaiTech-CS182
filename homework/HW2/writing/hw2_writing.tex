...
\begin{enumerate}[1.]
	...
	where $y_c^i$ is the $c$-th element of $y^i$.
		  ...
		  If $c\neq 1$, then $\nabla_{\beta_1}\beta_c^Tx^i=0$, and if $c=1$, then $\nabla_{\beta_1}\beta_c^Tx^i=x^i$.\\
		  And $\nabla_{\beta_1}\log(\sum\limits_{c'}exp(\beta_{c'}^Tx^i))=\dfrac{1}{\sum\limits_{c'}\beta_{c'}^Tx^i}x^i$.\\
		  So
		  $$\nabla_{\beta_1}\ell(\beta)=\sum\limits_{i=1}^n\sum\limits_{c=1}^k\left(y_c^ix^i-y_c^i\cdot\dfrac{1}{\sum\limits_{c'}\beta_{c'}^Tx^i}x^i\right)$$
